...
export default function RunList({ runs, factors, molds, boilers, onRefresh, onOpenLot }) {
  ...
  async function loadSpecimenCounts() {
    ...
    if (!data) return;
    const counts = {};
    data.forEach(s => { counts[s.run_id] = (counts[s.run_id] || 0) + 1; });
    setSpecimenCounts(counts);
  }

  async function loadFixedFactors() {
    if (!runs.length) return;
    const { data } = await supabase
  ...
  function onMoldSelect(moldId) {
    const mold = activeMolds.find(m => m.id === Number(moldId));
    setForm({ ...form, mold_id: moldId });
  }
    ...
    <div className="space-y-4">
      ...
      {showCreate && (
        <div className="card space-y-3 border-purple-200 bg-purple-50/30">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select
              value={form.mold_id}
              onChange={e => onMoldSelect(e.target.value)}
              className="border rounded-lg px-3 py-2 text-sm"
            >
          ...
          {form.mold_id && (() => {
            ...
            return mold ? (
              <div className="flex gap-2 flex-wrap">
                ...
                {boiler && <span className="badge bg-blue-50 text-blue-700">{boiler.boiler_no}번 보일러 · {boiler.setting_temp}℃</span>}
              </div>
            ) : null;
          })()}
                    ...
                    <div key={f.id} className={`flex items-center gap-2 p-2 rounded-lg ${isActive ? 'bg-purple-100' : 'bg-gray-50'}`}>
                      ...
                      {isActive ? (
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            type="text"
                            value={form.active_factor_value}
                            onChange={e => setForm({ ...form, active_factor_value: e.target.value })}
                            placeholder="실험값"
                            className="w-20 text-xs text-center border rounded px-2 py-1"
                          />
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            type="text"
                            value={form.fixedValues[f.id] || ''}
                            onChange={e => setForm({
                              ...form,
                              fixedValues: { ...form.fixedValues, [f.id]: e.target.value }
                            })}
                            placeholder="고정값"
                            className="w-20 text-xs text-center border rounded px-2 py-1"
                          />
      ...
      {runs.length === 0 ? (
        <div className="card text-center py-12 text-gray-400 text-sm">
          ...
          실험 Run이 없습니다<br />위 버튼으로 첫 실험을 등록하세요
        </div>
      ) : (
        <div className="space-y-3">
          {runs.map(run => {
            ...
            return (
              <div
                key={run.id}
                className="card cursor-pointer hover:border-purple-300 transition-colors"
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {run.boiler_no && (
                      <span className="text-xs text-gray-400">{run.boiler_no}번 보일러</span>
                    )}
                ...
                {run.active_factor_name && (
                  <div className="flex gap-1 flex-wrap mb-2">
                    ...
                    {fixed.map(ff => (
                      <span key={ff.id} className="badge badge-lock">
                        🔒 {ff.factor_name}: {ff.fixed_value}
                      </span>
                    ))}
                  </div>
                )}
                ...
                {run.memo && (
                  <div className="text-xs text-gray-400 mb-2">{run.memo}</div>
                )}
